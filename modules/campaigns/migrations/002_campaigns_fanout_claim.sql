-- ============================================================================
-- Module: campaigns
-- Migration: 002_campaigns_fanout_claim
-- Description: The send-engine RPCs for campaigns — direct clones of the proven
...
-- one table-parametric engine (spec-campaigns-module.md Phase 0). Until then
-- campaigns ride the same drip pattern newsletters use in production today.
-- ============================================================================

-- 1. Atomic dispatcher claim: pending rows whose send_at has arrived, soonest
--    first, ONLY for actively 'sending' campaigns (so paused/scheduled/
...
CREATE OR REPLACE FUNCTION public.claim_due_campaign_recipients(p_limit integer DEFAULT 500)
RETURNS SETOF public.campaign_send_recipients
LANGUAGE sql
AS $$
  UPDATE public.campaign_send_recipients r
  SET status = 'sending', attempts = r.attempts + 1, updated_at = now()
  FROM (
    SELECT csr.id
    FROM public.campaign_send_recipients csr
    JOIN public.campaign_sends s ON s.id = csr.send_id
...
COMMENT ON FUNCTION public.claim_due_campaign_recipients(integer) IS
  'Atomically claim due campaign_send_recipients (pending→sending) for the dispatcher; gated on parent status=sending; FOR UPDATE SKIP LOCKED makes overlapping ticks / replicas safe.';

-- 2. Fan-out: materialise one campaign_send_recipients row per audience member,
...
CREATE OR REPLACE FUNCTION public.fanout_campaign_send_recipients(p_send_id uuid)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_send.id IS NULL THEN
    RAISE EXCEPTION 'campaign_send % not found', p_send_id;
  END IF;

  IF v_send.audience_type = 'segment' AND v_send.segment_id IS NULL THEN
    RAISE EXCEPTION 'campaign_send % has audience_type=segment but no segment_id', p_send_id;
  END IF;
  IF v_send.audience_type = 'list' AND COALESCE(array_length(v_send.list_ids, 1), 0) = 0 THEN
    RAISE EXCEPTION 'campaign_send % has audience_type=list but no list_ids', p_send_id;
  END IF;
  ...
  INSERT INTO public.campaign_send_recipients (send_id, person_id, email, send_at, status, strategy, timezone)
  SELECT
    p_send_id,
    aud.person_id,
    aud.email,
  ...
  WHERE
    ...
    -- Exclude recipients already sent in a prior campaign send. sent_at (not
    ...
    AND (
      ...
      OR NOT EXISTS (
        ...
        WHERE esl.campaign_send_id = ANY (v_send.exclude_sent_send_ids)
  ...
  SET total_recipients = (SELECT count(*) FROM public.campaign_send_recipients WHERE send_id = p_send_id),
      updated_at = now()
  WHERE id = p_send_id;
...
COMMENT ON FUNCTION public.fanout_campaign_send_recipients(uuid) IS
  'Materialise per-recipient send_at rows for a campaign send from segment membership (or contact lists). target_local resolved per recipient timezone; suppression-filtered; excludes prior-send recipients via email_send_log.sent_at. Idempotent. Dispatcher drips via claim_due_campaign_recipients.';

-- 3. Per-timezone breakdown for the Sending UI (cf.
--    newsletter_send_timezone_breakdown).
CREATE OR REPLACE FUNCTION public.campaign_send_timezone_breakdown(p_send_id uuid)
RETURNS TABLE (
  timezone   text,
  recipients bigint,
  sent       bigint,
...
AS $$
  ...
  FROM public.campaign_send_recipients r
  WHERE r.send_id = p_send_id
  GROUP BY COALESCE(r.timezone, 'UTC')
  ORDER BY min(r.send_at);
$$;

COMMENT ON FUNCTION public.campaign_send_timezone_breakdown(uuid) IS
  'Per-timezone status breakdown for a staggered campaign send, ordered by dispatch time.';
