...
export default function FormPage({ params, apiUrl }: { params: { slug: string }; apiUrl?: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // /api/forms/* (no /modules/ segment) is JWT-exempt. The
      // /api/modules/* prefix on the platform API server has
      // requireJwt() middleware, which would 401 the anonymous
      // form submitter.
      const res = await fetch(`${baseApiUrl}/api/forms/${form.slug}/submit`, {
        method: 'POST',
