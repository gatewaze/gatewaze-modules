...
interface FormField {
  ...
  options?: string[];
  /**
   * Marks the field as contract-locked by another module. When set:
   *   - the field key (`id`) and `type` are immutable (rename / type change disabled)
   *   - the delete action is disabled
   *   - the `label`, `placeholder`, `required`, and `options` remain editable
   * Other modules can rely on this field key being present in submissions.
   * See modules/forms/guide.md "Module-locked fields" for the full contract.
   */
  locked_by_module?: string;
}
...
export default function FormDetailPage() {
  ...
  const subTabs: Tab[] = [
    { id: 'builder', label: 'Builder', icon: <PencilSquareIcon className="size-4" /> },
    { id: 'submissions', label: `Submissions (${submissions.length || 0})`, icon: <TableCellsIcon className="size-4" /> },
    { id: 'embed', label: 'Share & Embed', icon: <CodeBracketIcon className="size-4" /> },
  ];
  ...
  return (
    <Page title={name || 'Form'}>
      <WorkspaceLayout
        ...
        subTabs={subTabs}
        activeSubTabId={viewMode}
        onSubTabChange={(v) => setViewMode(v as ViewMode)}
        actions={
          viewMode === 'builder' ? (
                ...
                {contentCategories.length > 0 && (
                  <div>
                    ...
                    <select
                      ...
                      className="w-full rounded-md border border-[var(--gray-a6)] bg-[var(--gray-a2)] px-3 py-2 text-sm text-[var(--gray-12)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-8)]"
                    >
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              {fields.length === 0 ? (
                <Card variant="surface" className="p-12 text-center">
                  ...
                  <Button onClick={addField} style={{ marginTop: "1rem" }} className="gap-2">
                    <PlusIcon className="size-4" />
                    Add First Field
                  </Button>
                </Card>
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-1.5">
                              ...
                              {field.locked_by_module && (
                                <span
                                  title={`Locked by the "${field.locked_by_module}" module — the field key and type cannot be changed and the field cannot be deleted. Other modules depend on this key being present in submissions. Label, placeholder, required, and options remain editable.`}
                                  className="inline-flex items-center"
                                >
                                  <LockClosedIcon className="size-3.5 text-[var(--gray-a9)]" />
                                </span>
                              )}
                            </div>
                            <div className="flex items-center gap-2 mt-0.5">
                              ...
                              {field.locked_by_module && (
                                <Badge color="neutral">Locked: {field.locked_by_module}</Badge>
                              )}
                            </div>
                          </div>
                        </div>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            if (field.locked_by_module) {
                              toast.error(`This field is locked by the "${field.locked_by_module}" module and cannot be removed.`);
                              return;
                            }
                            removeField(field.id);
                          }}
                          disabled={!!field.locked_by_module}
                          title={field.locked_by_module ? `Locked by the "${field.locked_by_module}" module` : 'Delete field'}
                          className="p-1 text-[var(--gray-a8)] hover:text-red-500 disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:text-[var(--gray-a8)]"
                        >
                      ...
                      {editingFieldId === field.id && (
                        <div className="mt-4 pt-4 border-t border-[var(--gray-a6)] space-y-3" onClick={(e) => e.stopPropagation()}>
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            <div>
                              <label className="block text-xs font-medium text-[var(--gray-11)] mb-1">
                                Type
                                {field.locked_by_module && (
                                  <span className="ml-1 text-[var(--gray-a8)]" title={`Locked by the "${field.locked_by_module}" module`}>(locked)</span>
                                )}
                              </label>
                              <select
                                value={field.type}
                                ...
                                className="w-full rounded-md border border-[var(--gray-a6)] bg-[var(--gray-a2)] px-3 py-2 text-sm text-[var(--gray-12)] disabled:opacity-60 disabled:cursor-not-allowed"
                              >
                                {FIELD_TYPES.map(t => (
                          ...
                          <div>
                            ...
                            <Input
                              value={field.id}
                              readOnly={!!field.locked_by_module}
                              onChange={(e) => {
                                if (field.locked_by_module) return;
                              ...
                              className={field.locked_by_module ? 'opacity-60 cursor-not-allowed' : ''}
                            />
                            <p className="text-xs text-[var(--gray-a8)] mt-1">
                              {field.locked_by_module
                                ? `This field key is required by the "${field.locked_by_module}" module and cannot be changed.`
                                : 'Unique identifier used in API responses'}
                            </p>
                          </div>
                        </div>
                      )}
