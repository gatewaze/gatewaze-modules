...
/**
 * Local requireJwt for the warehouse-sync module.
 *
 ...
 * gate for warehouse-sync's admin API, which can enumerate Airbyte connections
 * and trigger syncs. It must verify token signatures — trusting an unverified
...
function getJwtSecret(): string {
  ...
  if (!secret) {
    throw new Error('JWT secret (SUPABASE_JWT_SECRET/JWT_SECRET) not set; warehouse-sync requireJwt cannot verify tokens');
  }
  return secret;
}
...
export function requireJwt() {
  ...
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      console.error('[warehouse-sync requireJwt] auth gate threw; denying request', err);
