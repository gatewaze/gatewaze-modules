...
// Resolve any avatar_url fields on a speaker-shaped object through the bucket URL.
// Idempotent: passes through values that are already full URLs.
function wrapSpeakerAvatar<T extends { avatar_url?: string | null | undefined }>(speaker: T): T {
  if (!speaker || !speaker.avatar_url) return speaker;
  return { ...speaker, avatar_url: toPublicUrl(speaker.avatar_url, BUCKET_URL) ?? undefined };
}
...
export function EventAgendaTab({ eventUuid, eventStart, eventEnd, talkDurationOptions, onTalkDurationOptionsChange }: EventAgendaTabProps) {
  ...
  const loadData = async () => {
    try {
      ...
      // Wrap avatar_url values so relative storage paths resolve to public URLs.
      setSpeakers(speakersData.map(wrapSpeakerAvatar));
      setConfirmedDurationCounts(durationCounts);
      setTalks(
        talksData.map(t => ({
          ...t,
          speakers: (t.speakers ?? []).map(wrapSpeakerAvatar),
        }))
      );
      ...
      if (entriesData.length > 0) {
        try {
          ...
          // Wrap avatar_url across all entries in the map.
          const wrappedMap: typeof speakersMap = {} as typeof speakersMap;
          for (const [entryId, entrySpeakers] of Object.entries(speakersMap)) {
            wrappedMap[entryId] = (entrySpeakers as any[]).map(wrapSpeakerAvatar);
          }
          setEntrySpeakers(wrappedMap);
        } catch {
