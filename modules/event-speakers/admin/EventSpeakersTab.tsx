...
function resolveAvatarUrl(avatarUrl: string | null | undefined): string | null {
  ...
  // Storage path → resolve via the media bucket
  return `${supabaseUrl}/storage/v1/object/public/media/${avatarUrl}`;
}
...
export function EventSpeakersTab({ eventUuid, eventId, eventLink, eventTitle, talkDurationOptions }: EventSpeakersTabProps) {
      ...
      talkSpeakers.forEach(speaker => {
        ...
        if (existing) {
          ...
          speakerMap.set(speaker.speaker_id, {
            ...
            avatarUrl: resolveAvatarUrl(speaker.avatar_url),
  ...
  // Probe for redirects module on mount
  useEffect(() => {
    if (linksAvailable !== null) return;
    (async () => {
      const { error } = await supabase.from('redirects').select('id').limit(1);
      if (error && (error.code === 'PGRST204' || error.code === 'PGRST205' || error.message?.includes('schema cache'))) {
  ...
  // Load speaker tracking links when confirmed/approved talks change (only if module available)
  useEffect(() => {
    if (linksAvailable !== true) return;
    if (confirmedTalks.length > 0 || approvedTalks.length > 0) {
      loadSpeakerLinks();
    }
  }, [confirmedTalks, approvedTalks, linksAvailable]);
  ...
  const handleAddNewSpeaker = async (asPending = false) => {
    ...
    try {
      ...
      if (!customer) {
        // Create person via people-signup edge function
        customer = await BulkRegistrationService.createPersonViaSignup(
      ...
      await BulkRegistrationService.updatePersonAttributes(customer.id, attributes);
    ...
    <Card key={talk.id} className="p-4 cursor-pointer hover:shadow-md transition-shadow h-full flex flex-col" onClick={() => setViewingTalk(talk)}>
      ...
      <div className="flex items-start gap-4 flex-1">
        ...
        <div className="shrink-0 relative">
          {resolveAvatarUrl(speaker?.avatar_url) ? (
            <img
        ...
        <div className="flex-1 min-w-0">
          ...
          {linksAvailable === true && (viewMode === 'confirmed' || viewMode === 'approved') && speakerId && speakerLinks[speakerId] && (
    ...
    try {
      ...
      for (const talk of confirmedTalks) {
        for (const speaker of (talk.speakers || [])) {
          const resolved = resolveAvatarUrl(speaker.avatar_url);
          if (!resolved || seenIds.has(speaker.speaker_id)) continue;
        ...
        (() => {
          ...
          const sorted = [...speakers].sort((a, b) => {
            const countCompleted = (s: typeof a) => {
              ...
              if (linksAvailable === true && s.talks.some(t => t.tracking_link_copied_at)) count++;
              return count;
            };
            ...
            <Card key={speakerGroup.speakerId} className="p-4">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* Tracking link stats */}
                  {linksAvailable === true && (viewMode === 'confirmed' || viewMode === 'approved') && speakerLinks[speakerGroup.speakerId] && (
      ...
      >
        {viewingTalk && (() => {
          ...
          <div className="space-y-6">
            {/* Speaker Header */}
            <div className="flex items-start gap-4">
              {resolveAvatarUrl(primarySpeaker?.avatar_url) ? (
                <img
            ...
            {linksAvailable === true && (viewingTalk.status === 'confirmed' || viewingTalk.status === 'approved') && primarySpeaker?.speaker_id && speakerLinks[primarySpeaker.speaker_id] && (
              <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                <h4 className="text-sm font-semibold text-gray-900 dark:text-white mb-3 flex items-center gap-2">
