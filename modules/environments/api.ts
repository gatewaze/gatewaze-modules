...
// `await import('@supabase/supabase-js')` from this file fails because
// the file lives at /premium-gatewaze-modules/modules/environments/api.ts
// (mounted from the module repo, not /app) and Node's bare-specifier
// resolution walks up from that path and never reaches the api
// container's /app/node_modules where the package actually lives.
//
// Use createRequire from a known-resolvable path inside the api server
// to bypass that. Try a few candidate paths so we work in dev (cwd is
// the workspace root) and prod (cwd is /app).
let _createClient: any = null;
function loadCreateClient(): any {
  ...
  const candidates = [
    ...
    '/app',
    '/app/packages/api',
    resolve(PROJECT_ROOT, 'packages/api'),
    PROJECT_ROOT,
  ];
  for (const cand of candidates) {
    try {
      ...
      tried.push(`${cand}: ${err.message?.split('\n')[0] ?? err}`);
    }
  }
  throw new Error(
    `Could not resolve @supabase/supabase-js from any known path. Tried: ${tried.join(' | ')}`,
  );
}
...
export function registerRoutes(app: Express, context?: ModuleContext) {
  ...
  app.post('/api/environments/sync', async (req: Request, res: Response) => {
    try {
      ...
      // Node's fetch throws with the maximally-unhelpful "fetch failed"
      // and tucks the real reason (DNS lookup, ECONNREFUSED, TLS, etc.)
      // into err.cause. Surface that so operators can actually debug
      // their docker / hostname setup.
      const describeFetchError = (err: any, action: string): string => {
        const cause = err?.cause;
        const causeMsg = cause?.message ?? cause?.code;
        const stem = err?.message ?? String(err);
        return causeMsg ? `${action}: ${stem} — ${causeMsg}` : `${action}: ${stem}`;
      };

      // Persist current state of the sync to the operation row. The
      // admin UI polls GET /api/environments/sync/:operationId every
      // few seconds and reads `log` + counters off this row, so without
      // periodic flushes the UI sits silent until completion. Throttled
      // to once every ~2s to avoid hammering Supabase mid-sync.
      let lastFlushAt = 0;
      ...
      try {
        ...
        for (const tableName of tables) {
          ...
          // PostgREST + Supabase Cloud cap a single response at 1000
          // rows by default (PGRST_MAX_ROWS), so a naive
          // /rest/v1/<table>?select=* silently truncates large tables.
          // Page through with the Range header — supabase-friendly,
          // doesn't require a known order column, and gives us the
          // total via Content-Range so we can stop cleanly.
          ...
          const pageSize = 1000;          // matches default PGRST_MAX_ROWS
          let pageStart = 0;
          let totalRows: number | null = null;
          let foundRowsForTable = 0;
          let tableErrored = false;

          // Outer loop over pages of source rows. We stream each page
          // straight into the destination so memory stays bounded
          ...
          while (true) {
            ...
            try {
              fetchRes = await fetch(
                `${source.supabase_url}/rest/v1/${tableName}?select=*`,
                {
                  headers: {
                    ...
                    // Range-Unit + Range tells PostgREST which slice we
                    // want; Prefer: count=exact makes it return the
                    // total in Content-Range so we know when to stop.
                    'Range-Unit': 'items',
              ...
              addLog('error', describeFetchError(err, `read ${tableName} from ${source.supabase_url}`));
              tableErrored = true;
              break;
            }

            // 206 Partial Content (PostgREST returns this for ranged
            // GETs) and 200 OK are both happy.
            if (!fetchRes.ok && fetchRes.status !== 206) {
              const body = await fetchRes.text().catch(() => '');
              addLog('error', `Failed to read ${tableName}: HTTP ${fetchRes.status} ${body.slice(0, 200)}`);
              tableErrored = true;
              break;
            }
            ...
            // Pull the total off Content-Range: "0-999/12345" — the
            // bit after the slash. Falls back to "*" if the source
            // can't compute it (rare; we then loop until we get an
            // under-full page).
            if (totalRows === null) {
              const contentRange = fetchRes.headers.get('content-range') ?? '';
              const match = contentRange.match(/\/(\d+|\*)$/);
              totalRows = match && match[1] !== '*' ? parseInt(match[1], 10) : -1;
              if (totalRows >= 0) {
                addLog('info', `Found ${totalRows} rows`);
              }
            }

            if (rows.length === 0) break;
            foundRowsForTable += rows.length;

            // Send this page to the destination in 500-row batches
            // (PostgREST POST is happy with much larger but 500 keeps
            // request bodies modest and individual failures small).
            const batchSize = 500;
            for (let i = 0; i < rows.length; i += batchSize) {
              ...
              try {
                ...
                addLog('error', describeFetchError(err, `insert into ${target.supabase_url}/${tableName}`));
                continue;
              }
              ...
              if (insertRes.ok) {
                ...
                addLog('warn', `Batch for ${tableName} returned ${insertRes.status}: ${body.slice(0, 200)}`);
              }
            ...
            // Stop when we've fetched a short page (last page reached)
            // or when we've covered the known total.
            if (rows.length < pageSize) break;
            if (totalRows >= 0 && pageStart + rows.length >= totalRows) break;
            pageStart += pageSize;
          }

          if (!tableErrored) {
            addLog('success', `Completed ${tableName}: ${foundRowsForTable} rows`);
          }
          await flushProgress();
        }
  ...
  /**
   * GET /api/environments/tables[?environmentId=…]
   *
   * Lists user-table names from the target Supabase instance. When
   * `environmentId` is supplied, fetches the schema from that
   * environment's URL using its service role key — the only pattern
   * that works for sync: the admin needs to see the SOURCE's tables to
   * choose what to pull, not its own tables.
   *
   * When no environmentId is supplied, falls back to the local
   * Supabase (preserves prior behaviour).
   */
  app.get('/api/environments/tables', async (req: Request, res: Response) => {
    try {
      const environmentId = (req.query.environmentId as string | undefined) ?? null;
      ...
      if (environmentId) {
        ...
        if (!url || !key) {
          return res.status(400).json({ error: 'Environment is missing url or service role key' });
        }
      } else {
        ({ url, key } = getLocalSupabaseCredentials());
        if (!url || !key) {
          return res.status(500).json({ error: 'Missing Supabase credentials' });
        }
      }
  ...
  /**
   * GET /api/environments/buckets[?environmentId=…]
   *
   * Lists storage bucket names from the target Supabase instance. Same
   * environmentId semantics as /tables: the admin sync UI needs to
   * surface the SOURCE's buckets, not its own (and listing buckets
   * requires the service role key, not the user's session token, which
   * is what `supabase.storage.listBuckets()` from the browser would
   * use).
   */
  app.get('/api/environments/buckets', async (req: Request, res: Response) => {
    try {
      const environmentId = (req.query.environmentId as string | undefined) ?? null;
      ...
      if (!response.ok) {
        ...
        return res.status(response.status).json({
          error: `Failed to list buckets (${response.status}): ${body.slice(0, 200)}`,
        });
      }

      const data = await response.json();
      const buckets = Array.isArray(data) ? data.map((b: any) => b.name).filter(Boolean) : [];
