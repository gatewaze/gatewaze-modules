    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        attributes: {
          ...
          marketing_consent: true, // Gradual is exclusively for a single community, so consent is assumed
          last_gradual_sync: now,
        },
        last_synced_at: now,
      })
...
async function handleUserRegistersForEvent(payload: UserRegistersForEventPayload): Promise<Response> {
  ...
  // Gradual is exclusively for a single community, so treat as consented
