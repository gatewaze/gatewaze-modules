...
import { ScraperService, Scraper, ScraperJob, ScraperHealth } from '@/utils/scraperService';
...
export default function ScrapersPage() {
  ...
  const [deletingScraper, setDeletingScraper] = useState(false);
  ...
  useEffect(() => {
    loadData();
    loadActiveJobsCount();
    const activeInterval = setInterval(loadActiveJobsCount, 10000);
    const healthInterval = setInterval(async () => {
      const result = await ScraperService.getHealth();
      if (result.data) setHealth(result.data);
    }, 30000);
    // Silent auto-refresh every 5s when there are running/pending jobs. Passes
    // silent=true so the loading spinner doesn't flash the whole page on each
    // tick — the previous behaviour made the Scrapers and Jobs tabs appear
    // to "refresh continually" because setLoading(true) unmounted the tables.
    const dataInterval = setInterval(() => {
      const hasActiveJobs = health?.running_jobs && health.running_jobs > 0;
      const hasPendingInView = recentJobs.some(j => j.status === 'pending' || j.status === 'running');
      if (hasActiveJobs || hasPendingInView) {
        loadData({ silent: true });
      }
    }, 5000);
    return () => { clearInterval(activeInterval); clearInterval(healthInterval); clearInterval(dataInterval); };
  }, [jobsPage, sortColumn, sortDirection, statusFilter]);

  const loadData = async (opts?: { silent?: boolean }) => {
    if (!opts?.silent) setLoading(true);
    try {
      const [scrapersResult, jobsResult, statsResult, healthResult] = await Promise.all([
  ...
  return (
    ...
    <div className="p-6">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
        <Card variant="surface" className="p-4" style={{ minWidth: 0 }}>
          <div className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Enabled</div>
          <div className="text-2xl font-bold">{health?.enabled_scrapers ?? stats.enabled_scrapers}</div>
        </Card>
        <Card variant="surface" className="p-4" style={{ minWidth: 0 }}>
          <div className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Running</div>
          <div className="text-2xl font-bold text-blue-600">{health?.running_jobs ?? 0}</div>
        </Card>
        <Card variant="surface" className={`p-4 ${(health?.stuck_jobs ?? 0) > 0 ? 'ring-2 ring-red-400 bg-red-50' : ''}`} style={{ minWidth: 0 }}>
          <div className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Stuck</div>
          <div className={`text-2xl font-bold ${(health?.stuck_jobs ?? 0) > 0 ? 'text-red-600' : 'text-green-600'}`}>
            {health?.stuck_jobs ?? 0} {(health?.stuck_jobs ?? 0) === 0 && <CheckCircleIcon className="inline h-5 w-5" />}
          </div>
        </Card>
        <Card variant="surface" className="p-4" style={{ minWidth: 0 }}>
          <div className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Failed (24h)</div>
          <div className={`text-2xl font-bold ${(health?.failed_24h ?? 0) > 0 ? 'text-amber-600' : ''}`}>{health?.failed_24h ?? 0}</div>
        </Card>
        <Card variant="surface" className="p-4" style={{ minWidth: 0 }}>
          <div className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Completed (24h)</div>
          <div className="text-2xl font-bold text-green-600">{health?.completed_24h ?? 0}</div>
        </Card>
        <Card variant="surface" className="p-4" style={{ minWidth: 0 }}>
          <div className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Scheduler</div>
          <div className="text-sm font-bold mt-1">
            {health?.scheduler_healthy === true && <span className="text-green-600"><CheckCircleIcon className="inline h-5 w-5 mr-1" />Running</span>}
            {health?.scheduler_healthy === false && <span className="text-red-600"><ExclamationTriangleIcon className="inline h-5 w-5 mr-1" />Down</span>}
            {health?.scheduler_healthy == null && <span className="text-neutral-400">Unknown</span>}
          </div>
        </Card>
      </div>
              ...
              <Tr key={scraper.id}>
                ...
                <Td>
                  {scraper.total_items_scraped > 0 ? (
                    <Button
                      variant="ghost"
                      color="blue"
                      onClick={() => window.location.href = `/events?scrapedBy=${encodeURIComponent(scraper.name)}`}
                    >
                ...
                <Td data-sticky-right style={{ position: 'sticky', right: 0, background: 'var(--color-panel-solid)', zIndex: 1 }}>
                  <RowActions actions={[
                    ...
                    { label: "Delete", icon: <TrashIcon className="size-4" />, onClick: () => setScraperToDelete(scraper), color: 'red' },
                  ]} />
                </Td>
              </Tr>
            ))}
                  ...
                  <Tr key={job.id}>
                    ...
                    <Td data-sticky-right style={{ position: 'sticky', right: 0, background: 'var(--color-panel-solid)', zIndex: 1 }}>
                      <RowActions actions={[
                        ...
                        ...(job.status === 'failed' ? [{
                          label: "Retry", icon: <ArrowPathIcon className="size-4" />,
                          onClick: async () => {
                            const result = await ScraperService.retryJob(job.id);
                            if (result.success) { loadData(); } else { alert(`Retry failed: ${result.error}`); }
                          }
                        }] : []),
      ...
      {/* Delete Scraper Confirmation Modal */}
      <ConfirmModal
        isOpen={scraperToDelete !== null}
        onClose={() => !deletingScraper && setScraperToDelete(null)}
        onConfirm={async () => {
          if (!scraperToDelete) return;
          setDeletingScraper(true);
          try {
            const result = await ScraperService.deleteScraper(scraperToDelete.id);
            if (!result.success) {
              alert(`Failed to delete scraper: ${result.error}`);
            } else {
              await loadData();
            }
          } finally {
            setDeletingScraper(false);
        ...
        message={
          scraperToDelete
            ? `Are you sure you want to delete "${scraperToDelete.name}"? All of its jobs and logs will be removed. Any in-flight runs will be cancelled. This action cannot be undone.`
            : ''
        }
        confirmText={deletingScraper ? 'Deleting…' : 'Delete'}
        confirmColor="red"
      />
    </div>
    </Page>
