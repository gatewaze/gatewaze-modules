...
export interface VariantComparisonPair {
  slow_id: number;
  slow_name: string;
  slow_type: string;
  ...
  slow_runs: number;
  fast_runs: number;
  slow_avg_duration_s: number | null;
  fast_avg_duration_s: number | null;
  slow_avg_items: number | null;
  fast_avg_items: number | null;
  slow_success_rate: number | null;
  fast_success_rate: number | null;
}
...
export interface ScraperHealth {
  ...
  scheduler_last_run: number | null;
...
export class ScraperService {
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          ...(scraper.content_category ? { content_category: scraper.content_category } : {}),
  ...
  static async updateScraper(scraperId: number, updates: Partial<Scraper>): Promise<{ data: Scraper | null; error: any }> {
    try {
      // Only include fields that are actual DB columns to avoid schema cache errors
      const updateData: Record<string, unknown> = { updated_at: new Date().toISOString() };
      const allowedFields = [
        'name', 'description', 'scraper_type', 'event_type', 'base_url',
        'enabled', 'config', 'timezone', 'schedule_enabled', 'schedule_frequency',
        'schedule_time', 'schedule_days', 'schedule_cron', 'object_type', 'account',
        'content_category', 'timeout_minutes', 'alert_on_failure',
      ];
      ...
      if (error) {
        // If content_category column doesn't exist yet, retry without it
        if (error.message?.includes('content_category')) {
          delete updateData.content_category;
          const retry = await supabase
            ...
            .single();
          if (retry.error) {
            console.error('Error updating scraper:', retry.error);
  ...
  /**
   * Delete a whole scraper — cascades to its jobs + logs via the DB FK, and
   * also evicts any BullMQ entries so an in-flight run doesn't outlive the
   * scraper row. Calls the API rather than hitting Supabase directly so the
   * BullMQ cleanup runs server-side.
   */
  static async deleteScraper(scraperId: number): Promise<{ success: boolean; error: any }> {
    try {
      const res = await fetch(`${getApiBaseUrl()}/scrapers/${scraperId}`, { method: 'DELETE' });
      ...
      console.error('Error in deleteScraper:', error);
      return { success: false, error };
    }
  }

  /**
   * Delete a scraper job — routes through the API so the BullMQ/Redis entry
   * for the job is removed alongside the DB row. Direct Supabase deletion
   * leaves the queue entry behind and the worker then spins on a job whose
   * DB row no longer exists.
   */
  static async deleteJob(jobId: number): Promise<{ success: boolean; error: any }> {
    try {
      const res = await fetch(`${getApiBaseUrl()}/scrapers/jobs/${jobId}`, { method: 'DELETE' });
      const body = await res.json().catch(() => ({}));
      if (!res.ok || body.success === false) {
        return { success: false, error: body.error || `HTTP ${res.status}` };
      }
  ...
  /**
   * Delete multiple scraper jobs (+ their BullMQ entries).
   */
  static async deleteJobs(jobIds: number[]): Promise<{ success: boolean; error: any; deleted?: number }> {
    try {
      const res = await fetch(`${getApiBaseUrl()}/scrapers/jobs/bulk-delete`, {
        method: 'POST',
      ...
      const body = await res.json().catch(() => ({}));
      if (!res.ok || body.success === false) {
        return { success: false, error: body.error || `HTTP ${res.status}`, deleted: 0 };
      }
      return { success: true, error: null, deleted: body.deleted || 0 };
    } catch (error) {
  ...
  /**
   * Delete all pending jobs (+ their BullMQ entries).
   */
  ...
  static async getActiveJobs(): Promise<{ data: any[] | null; error: any }> {
    try {
      ...
      // Map API format to ActiveJob format expected by UI
      const jobs = (data.jobs || []).map((j: any) => ({
        jobId: j.data?.scraperJobId || j.id,
        status: j.status || 'unknown',
        startedAt: j.createdAt ? new Date(j.createdAt).toISOString() : '',
        scraperName: j.data?.scraperName || 'Unknown',
  ...
  /**
   * Get scraper health summary for the dashboard
   */
  static async getHealth(): Promise<{ data: ScraperHealth | null; error: any }> {
    try {
      const apiBase = getApiBase();
      const response = await fetch(`${apiBase}/scrapers/health`, {
        headers: { 'Content-Type': 'application/json' },
      });
      const result = await response.json();
      if (!result.success) return { data: null, error: result.error };
      return { data: result.health, error: null };
    } catch (error) {
  ...
  /**
   * Slow vs Fast comparison data for paired Luma scrapers.
   * See spec-scrapling-fetcher-service.md §4.6.
   */
  static async getVariantComparison(
    windowDays = 7,
  ): Promise<{ data: VariantComparisonPair[] | null; error: any }> {
    try {
      const apiBase = getApiBase();
      const response = await fetch(
        `${apiBase}/scrapers/comparison?window_days=${windowDays}`,
        { headers: { 'Content-Type': 'application/json' } },
      );
      const result = await response.json();
      if (!result.success) return { data: null, error: result.error };
      return { data: result.pairs, error: null };
    } catch (error) {
      console.error('Error in getVariantComparison:', error);
      return { data: null, error };
    }
  }
   ...
   * Retry a failed job
   */
  static async retryJob(jobId: number): Promise<{ success: boolean; newJobId?: number; error: any }> {
    try {
      const apiBase = getApiBase();
      const response = await fetch(`${apiBase}/scrapers/${jobId}/retry`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      const result = await response.json();
      if (!result.success) return { success: false, error: result.error };
      return { success: true, newJobId: result.jobId, error: null };
    } catch (error) {
