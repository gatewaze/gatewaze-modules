/**
 * LumaSearchScraperFast — Fast variant of LumaSearchScraper.
 *
 * Spec: gatewaze-environments/specs/spec-scrapling-fetcher-service.md §4.3.1.
 *
 * Behavior:
 *   - Inherits the discovery flow (Brave / Google CSE) from LumaSearchScraper.
...
export class LumaSearchScraperFast extends LumaSearchScraper {
    ...
    try {
      ...
      if (err instanceof ScraplingNotConfiguredError) {
        if (!this._fastFallbackLogged) {
          console.log(
            '🐢 scrapling-fetcher not configured; LumaSearchScraperFast falling back to browser path for this run',
          );
