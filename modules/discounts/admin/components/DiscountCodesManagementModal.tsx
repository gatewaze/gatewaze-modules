...
import { ArrowUpTrayIcon, DocumentTextIcon, CheckCircleIcon, UserGroupIcon, TicketIcon } from '@heroicons/react/24/outline';
    ...
    <Modal
      ...
      footer={
        <div className="flex justify-end gap-3">
          <Button onClick={handleClose} disabled={isProcessing} variant="outline">Cancel</Button>
          <Button
            variant="solid"
            onClick={handleSubmit}
            disabled={(activeTab === 'generate' ? !generateCount.trim() : !codesText.trim()) || isProcessing}
          >
            {isProcessing ? (<>Processing...</>) : (<>{tabContent.buttonText}</>)}
          </Button>
        </div>
      }
    >
      <p className="text-sm text-[var(--gray-9)] mb-4">{displayTitle}</p>

        {/* Tabs */}
        <div className="flex border-b border-[var(--gray-a6)] -mx-4 mb-4">
          <button
            onClick={() => handleTabChange('upload')}
            className={`flex-1 px-6 py-3 text-sm font-medium transition-colors ${
              activeTab === 'upload'
                ? 'text-[var(--accent-11)] border-b-2 border-[var(--accent-9)] bg-[var(--accent-2)]'
