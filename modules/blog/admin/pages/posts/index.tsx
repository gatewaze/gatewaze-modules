...
const postSchema = yup.object({
  ...
  featured_image_alt: yup.string().nullable().optional(),
  ...
  og_title: yup.string().max(60, 'OG title must be 60 characters or less').nullable().optional(),
  og_description: yup.string().max(160, 'OG description must be 160 characters or less').nullable().optional(),
  og_image: yup.string().nullable().optional(),
  twitter_title: yup.string().max(60, 'Twitter title must be 60 characters or less').nullable().optional(),
  twitter_description: yup.string().max(160, 'Twitter description must be 160 characters or less').nullable().optional(),
  twitter_image: yup.string().nullable().optional(),
  category_id: yup.string().nullable().optional(),
  content_category: yup.string().nullable().optional(),
  is_featured: yup.boolean().default(false),
  allow_comments: yup.boolean().default(true),
  scheduled_for: yup.string().nullable().optional(),
});
...
const BlogManagement: React.FC = () => {
  ...
  const [featuredImagePath, setFeaturedImagePath] = useState<string | undefined>(undefined);

  // UI state
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);
  const [showModal, setShowModal] = useState(false);
  const [modalType, setModalType] = useState<'post' | 'category' | 'tag' | null>(null);
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (categoriesResult.success && categoriesResult.data) {
        console.log('Setting categories:', categoriesResult.data.length);
        setCategories(categoriesResult.data);
      }
      if (tagsResult.success && tagsResult.data) {
        console.log('Setting tags:', tagsResult.data.length);
  ...
  const onPostSubmit = async (data: PostFormData) => {
    ...
    try {
      ...
      // Clean up data - convert empty strings to null for optional fields (to match database schema)
      const cleanedData = {
        ...data,
        category_id: data.category_id && data.category_id.trim() !== '' ? data.category_id : null,
        content_category: data.content_category && data.content_category.trim() !== '' ? data.content_category : null,
        featured_image: data.featured_image && data.featured_image.trim() !== '' ? data.featured_image : null,
        canonical_url: data.canonical_url && data.canonical_url.trim() !== '' ? data.canonical_url : null,
        og_image: data.og_image && data.og_image.trim() !== '' ? data.og_image : null,
        twitter_image: data.twitter_image && data.twitter_image.trim() !== '' ? data.twitter_image : null,
        password: data.password && data.password.trim() !== '' ? data.password : null,
        meta_title: data.meta_title && data.meta_title.trim() !== '' ? data.meta_title : null,
        meta_description: data.meta_description && data.meta_description.trim() !== '' ? data.meta_description : null,
        og_title: data.og_title && data.og_title.trim() !== '' ? data.og_title : null,
        og_description: data.og_description && data.og_description.trim() !== '' ? data.og_description : null,
        twitter_title: data.twitter_title && data.twitter_title.trim() !== '' ? data.twitter_title : null,
        twitter_description: data.twitter_description && data.twitter_description.trim() !== '' ? data.twitter_description : null,
        scheduled_for: data.scheduled_for && data.scheduled_for.trim() !== '' ? data.scheduled_for : null,
        featured_image_alt: data.featured_image_alt && data.featured_image_alt.trim() !== '' ? data.featured_image_alt : null,
      };

      if (editingPost) {
        result = await BlogPostsService.update(editingPost.id, cleanedData, selectedTags);
      } else {
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      console.log('Deleting category:', deleteCategory.id);
      const result = await BlogCategoriesService.delete(deleteCategory.id);
      console.log('Delete result:', result);

      if (result.success) {
        toast.success('Category deleted successfully');
        console.log('Categories before reload:', categories.length);

        // Optimistically update the UI by removing the deleted category
        setCategories(prevCategories =>
          prevCategories.filter(cat => cat.id !== deleteCategory.id)
        );

        // Also reload data as backup
  ...
  const handleDeleteTag = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Optimistically update the UI by removing the deleted tag
        setTags(prevTags =>
          prevTags.filter(tag => tag.id !== deleteTag.id)
        );
  ...
  const handleOpenPostModal = (post?: BlogPost) => {
    ...
    if (post) {
      ...
      const formFields: (keyof PostFormData)[] = [
        ...
        'category_id', 'content_category', 'is_featured', 'allow_comments', 'scheduled_for'
      ];
      ...
      formFields.forEach(field => {
        if (field in post) {
          ...
          postForm.setValue(field, value === null ? '' : value);
        }
      });
      setSelectedTags(post.tags?.map(tag => tag.id) || []);
      // Set featured image path if it exists
      setFeaturedImagePath(undefined); // Will be set by ImageUpload component
    } else {
      postForm.reset({
        status: 'draft',
        visibility: 'public',
      ...
      setSelectedTags([]);
      setFeaturedImagePath(undefined);
    }
    setShowModal(true);
  };
                  ...
                  <div className="space-y-4">
                    ...
                    {contentCategories.length > 0 && (
                      <div>
                        ...
                        <select
                          {...postForm.register('content_category')}
                          className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-800"
                        >
                ...
                <Card className="p-4">
                  ...
                  <div className="space-y-4">
                    <ImageUpload
                      ...
                      value={postForm.watch('featured_image') || ''}
                      onChange={(url) => {
                        postForm.setValue('featured_image', url || '');
                      }}
                      onImagePathChange={setFeaturedImagePath}
