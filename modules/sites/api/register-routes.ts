...
export function registerRoutes(app: Express, context?: ModuleContext): void {
  ...
  const buildSiteContentFiles = async (
    ...
    //     runtime API. Per spec-aaif-theme-deliverable §5.2.
    ...
    if (!themeOwnsRouting) {
      ...
      // stale stubs persist on aaif-publish:main and continue to shadow
  ...
  // Personas (per spec-aaif-theme-deliverable §5.2) — editor-managed
  ...
  // Runtime content API (per spec-aaif-theme-deliverable §7) — read
