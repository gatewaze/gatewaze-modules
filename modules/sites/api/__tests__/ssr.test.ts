...
const baseResolved = {
  ...
  site: { id: 's1', slug: 'example', name: 'EXAMPLE', config: {} },
...
describe('SSR getRender()', () => {
  it('returns 400 when host or path is missing', async () => {
    ...
    const res = fakeRes();
    await routes.getRender(fakeReq({ host: 'example.test' }), res);
  ...
  it('returns 400 on malformed path', async () => {
    const routes = createSsrRoutes(baseDeps());
    const res = fakeRes();
    await routes.getRender(fakeReq({ host: 'example.test', path: 'no-slash' }), res);
    expect(res._status).toBe(400);
  });
  ...
  it('returns 429 when rate-limited', async () => {
    ...
    const res = fakeRes();
    await routes.getRender(fakeReq({ host: 'example.test', path: '/' }), res);
