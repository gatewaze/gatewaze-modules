 ...
 *   - 200 /settings returns EXAMPLE defaults when branding config absent
...
function makeDeps(tables: Record<string, ScalarRow[]>): PublicMenusRoutesDeps & {
  ...
  return {
    ...
    supabasePublicUrl: 'http://supabase.example.localhost',
    storageBucket: 'media',
  };
}
...
describe('getNavigation', () => {
  ...
  it('returns 404 when menu not found under site', async () => {
    const deps = makeDeps({
      sites: [{ id: 'site-1', slug: 'example', name: 'EXAMPLE', config: {} }],
      navigation_menus: [],
    });
    const routes = createPublicMenusRoutes(deps);
    const req = { params: { siteSlug: 'example', menuSlug: 'primary' } } as unknown as Request;
    const { res, status, json } = makeRes();
...
describe('getSettings', () => {
  ...
  it('returns EXAMPLE socials + copyright when slug is example and config absent', async () => {
    const deps = makeDeps({
      sites: [{ id: 'site-1', slug: 'example', name: 'EXAMPLE', config: {} }],
      host_media: [],
    });
    const routes = createPublicMenusRoutes(deps);
    const req = { params: { siteSlug: 'example' } } as unknown as Request;
    const { res, status, json } = makeRes();
    ...
    expect(payload.socials.x).toContain('twitter.com/example');
  ...
  it('builds logo URL from host_media when no branding config', async () => {
      ...
      host_media: [
        {
          ...
          host_kind: 'site',
          host_id: 'site-1',
          storage_path: 'sites/site-1/media/example-secondary-logo-black.svg',
          filename: 'example-secondary-logo-black.svg',
    ...
    expect(payload.logo?.url).toContain('example-secondary-logo-black.svg');
