/**
 * Beehiiv Rich Text Output Adapter
 *
 * Generates simplified semantic HTML suitable for pasting into Beehiiv's
 * rich text editor. Excludes header, footer, and promotional blocks that
 * Beehiiv provides natively.
 */

import { renderTemplate } from '@/utils/templateParser';
// Cross-module helper — see substackOutputAdapter for the rationale.
...
/**
 * Block types excluded from Beehiiv output.
 * Beehiiv provides its own header/footer and we skip promotional blocks.
 */
...
export const BeehiivOutputAdapter: INewsletterOutputAdapter = {
  meta: {
    id: 'beehiiv',
    label: 'Beehiiv',
    description: 'Simplified semantic HTML for Beehiiv rich text editor',
  ...
  async render(context: OutputRenderContext): Promise<string> {
    ...
    // Per spec-builder-evaluation §3.6 (extended). Same dispatch shape as
    // Substack — when react-email blocks are present, route through
    // EditionEmail with format='beehiiv', strip the html/body shell,
    // run the existing wrapRichTextOutput post-processor.
    if (hasReactEmailBlocks(filteredCtx)) {
      const fullHtml = await renderViaEditionEmail({ context: filteredCtx, format: 'beehiiv' });
