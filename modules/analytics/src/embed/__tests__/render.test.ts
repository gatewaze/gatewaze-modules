...
describe('renderEmbed', () => {
  ...
  it('skips dimension pre-bake for external properties (resolved at runtime)', () => {
    const out = renderEmbed({
      propertyId: PROPERTY_ID,
      kind: 'external',
      ingestOrigin: 'https://analytics.aaif.live',
  ...
  it('strips trailing slash from ingestOrigin', () => {
    ...
    expect(out.head).toContain('data-host-url="https://aaif.live"');
    expect(out.head).not.toContain('aaif.live//');
  });
});
