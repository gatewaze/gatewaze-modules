        ...
        <div className="p-6">
          ...
          {attendance.length > 0 && (
            <>
              ...
              {(() => {
                const attendedRegIds = new Set(attendance.map((a: any) => a.event_registration_id).filter(Boolean));
                const codeStats = new Map<string, { code: string; source: string | null; registered: number; attended: number }>();
                for (const reg of registrations) {
                  const code = (reg as any).discount_code;
                  if (!code) continue;
                  if (!codeStats.has(code)) {
                    codeStats.set(code, { code, source: (reg as any).discount_code_source || null, registered: 0, attended: 0 });
                  }
                  const stat = codeStats.get(code)!;
                  stat.registered++;
                  if (attendedRegIds.has(reg.id)) stat.attended++;
                }
                const sortedCodeStats = [...codeStats.values()].sort((a, b) => {
                  const pctA = a.registered > 0 ? a.attended / a.registered : 0;
                  const pctB = b.registered > 0 ? b.attended / b.registered : 0;
                  return pctB - pctA;
                });
                if (sortedCodeStats.length === 0) return null;
                const totalWithCode = sortedCodeStats.reduce((s, c) => s + c.registered, 0);
                const totalAttendedWithCode = sortedCodeStats.reduce((s, c) => s + c.attended, 0);
                return (
                  <div className="p-4 bg-violet-50 dark:bg-violet-900/20 rounded-lg border border-violet-200 dark:border-violet-800 mb-6">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="text-sm font-semibold text-violet-900 dark:text-violet-100">Discount Code Conversion</h4>
                      <div className="text-xs text-violet-700 dark:text-violet-300">
                        {totalWithCode} registrations, {totalAttendedWithCode} attended ({totalWithCode > 0 ? Math.round((totalAttendedWithCode / totalWithCode) * 100) : 0}%)
                      </div>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="min-w-full">
                        <thead>
                          <tr className="text-xs text-violet-700 dark:text-violet-300 uppercase">
                            <th className="text-left py-1.5 pr-3 font-medium">Code</th>
                            <th className="text-left py-1.5 pr-3 font-medium">Source</th>
                            <th className="text-right py-1.5 pr-3 font-medium">Registered</th>
                            <th className="text-right py-1.5 pr-3 font-medium">Attended</th>
                            <th className="text-right py-1.5 pr-3 font-medium">Conversion</th>
                            <th className="py-1.5 w-28"></th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-violet-200 dark:divide-violet-800">
                          {sortedCodeStats.map((stat) => {
                            const pct = stat.registered > 0 ? Math.round((stat.attended / stat.registered) * 100) : 0;
                            return (
                              <tr key={stat.code}>
                                <td className="py-1.5 pr-3 text-sm font-mono text-violet-900 dark:text-violet-100">{stat.code}</td>
                                <td className="py-1.5 pr-3 text-sm text-violet-700 dark:text-violet-300">{stat.source || <span className="text-[var(--gray-a9)]">-</span>}</td>
                                <td className="py-1.5 pr-3 text-sm text-right font-semibold text-violet-900 dark:text-violet-100 tabular-nums">{stat.registered}</td>
                                <td className="py-1.5 pr-3 text-sm text-right font-semibold text-violet-900 dark:text-violet-100 tabular-nums">{stat.attended}</td>
                                <td className="py-1.5 pr-3 text-sm text-right font-semibold tabular-nums" style={{ color: pct >= 70 ? 'var(--green-11)' : pct >= 40 ? 'var(--yellow-11)' : 'var(--red-11)' }}>{pct}%</td>
                                <td className="py-1.5">
                                  <div className="w-28 h-2 bg-violet-200 dark:bg-violet-800 rounded-full overflow-hidden">
                                    <div className="h-full bg-violet-600 dark:bg-violet-400 rounded-full" style={{ width: `${pct}%` }} />
                                  </div>
                                </td>
                              </tr>
                            );
              ...
              {/* No-Shows */}
              {(() => {
                const attendedEmails = new Set(attendance.map((a: any) => a.email?.toLowerCase()).filter(Boolean));
                const noShows = registrations.filter((reg: any) =>
                  reg.status === 'confirmed' && !attendedEmails.has(reg.email?.toLowerCase())
                );
                if (noShows.length === 0) return null;
                const paidNoShows = noShows.filter((r: any) => r.amount_paid > 0);
                const totalRevenueLost = paidNoShows.reduce((sum: number, r: any) => sum + (r.amount_paid || 0), 0);
                return (
                  <div className="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800 mb-6">
                    <div className="flex items-center justify-between mb-3">
                      <h4 className="text-sm font-semibold text-amber-900 dark:text-amber-100">No-Shows</h4>
                      <div className="text-xs text-amber-700 dark:text-amber-300">
                        {noShows.length} registrant{noShows.length !== 1 ? 's' : ''}
                        {paidNoShows.length > 0 && <> &middot; {paidNoShows.length} paid ({new Intl.NumberFormat('en-US', { style: 'currency', currency: paidNoShows[0]?.currency || 'USD' }).format(totalRevenueLost)})</>}
                      </div>
                    </div>
                    <div className="overflow-x-auto max-h-96 overflow-y-auto">
                      <table className="min-w-full">
                        <thead className="sticky top-0 bg-amber-50 dark:bg-amber-900/20">
                          <tr className="text-xs text-amber-700 dark:text-amber-300 uppercase">
                            <th className="text-left py-1.5 pr-3 font-medium">Name</th>
                            <th className="text-left py-1.5 pr-3 font-medium">Email</th>
                            <th className="text-left py-1.5 pr-3 font-medium">Company</th>
                            <th className="text-right py-1.5 pr-3 font-medium">Amount Paid</th>
                            <th className="text-left py-1.5 pr-3 font-medium">Discount Code</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-amber-200 dark:divide-amber-800">
                          {noShows.sort((a: any, b: any) => (b.amount_paid || 0) - (a.amount_paid || 0)).map((reg: any) => (
                            <tr key={reg.id}>
                              <td className="py-1.5 pr-3 text-sm text-amber-900 dark:text-amber-100">{reg.full_name || '-'}</td>
                              <td className="py-1.5 pr-3 text-sm text-amber-700 dark:text-amber-300">{reg.email || '-'}</td>
                              <td className="py-1.5 pr-3 text-sm text-amber-700 dark:text-amber-300">{reg.company || '-'}</td>
                              <td className="py-1.5 pr-3 text-sm text-right font-semibold tabular-nums" style={{ color: (reg.amount_paid || 0) > 0 ? 'var(--amber-11)' : 'var(--gray-a9)' }}>
                                {reg.amount_paid > 0 ? new Intl.NumberFormat('en-US', { style: 'currency', currency: reg.currency || 'USD' }).format(reg.amount_paid) : 'Free'}
                              </td>
                              <td className="py-1.5 pr-3 text-sm text-amber-700 dark:text-amber-300 font-mono">{(reg as any).discount_code || '-'}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
