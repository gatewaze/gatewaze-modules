/**
 * Per-recipient weather resolution for send-time weather blocks — shared by the
 * newsletter AND broadcast worker bindings.
 *
 * Pure (no Deno APIs, only global fetch) so it's safe in the Node worker. Given
 * a recipient's city/country it geocodes + fetches the current forecast from
 * open-meteo (no API key) and returns the `{{weather_*}}` substitution values,
 * or a blank/fallback map when the location is unknown/unavailable. Callers
 * cache by `city|country|units`.
 *
 * Extracted verbatim from newsletters/workers/send-engine-binding.ts so both
