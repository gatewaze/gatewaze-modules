// @ts-nocheck
/**
 * Minimal in-memory sliding-window rate limiter (per API instance — not
 * distributed; adequate for a single module API process). Keyed by an arbitrary
 * string (client IP or user id). Mirrors modules/software-engineer/lib/rate-limit.ts
 * (kept local so newsletters carries no cross-module dependency).
 */
...
export function clientIp(req: unknown): string {
  ...
  // Prefer req.ip (Express `trust proxy` set): a public-client-forged
  // X-Forwarded-For is ignored. Raw XFF is only a spoofable fallback.
