/**
 * Newsletter block-level link tracking — pure helpers.
 * Per spec-newsletter-link-tracking.md (§4.2–4.4).
 *
 * Runtime-agnostic (no DB, no framework, no DOM): used by the admin editor,
 * the `newsletter-send` edge function (tagging), and the `email-webhook`
 * function (parsing). Keep it dependency-free.
 */
...
/** One trackable link occurrence within an edition, in document order. */
...
/** A registry row as needed for tagging (subset of newsletters_edition_links). */
...
/**
 * Extract every trackable link occurrence from an edition's blocks, in stable
...
export function extractTrackableLinks(blocks: ReadonlyArray<LinkSourceBlock>): LinkOccurrence[] {
  ...
  for (const block of sortedBlocks) {
    ...
    // swallows it, and the registry never gets populated. Sharing fieldCount
    // makes the second podcast.apple_link land at link_index=1, third at 2.
    // Brick identity is still recoverable via the brick_id column.
...
export function tagUrl(url: string, trackingKey: string): string {
  ...
  let path = qIdx >= 0 ? base.slice(0, qIdx) : base;
