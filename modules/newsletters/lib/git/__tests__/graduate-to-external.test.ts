...
describe('detectProvider', () => {
  it('recognises github.com https', () => {
    expect(detectProvider('https://github.com/gatewaze/example-theme.git')).toBe('github');
  });
  ...
  it('recognises github ssh form', () => {
    expect(detectProvider('git@github.com:gatewaze/example-theme.git')).toBe('github');
  });
...
describe('parseOwnerRepo', () => {
  it('parses github owner/repo', () => {
    expect(parseOwnerRepo('https://github.com/gatewaze/example-publish.git', 'github')).toBe(
      'gatewaze/example-publish',
    );
  });
  it('parses github owner/repo without .git suffix', () => {
    expect(parseOwnerRepo('https://github.com/gatewaze/example-publish', 'github')).toBe(
      'gatewaze/example-publish',
    );
  });
...
describe('toSshUrl', () => {
  it('converts github https → ssh form', () => {
    expect(toSshUrl('https://github.com/gatewaze/example-publish.git', 'github')).toBe(
      'git@github.com:gatewaze/example-publish.git',
    );
  });
  it('passes through already-ssh urls unchanged', () => {
    expect(toSshUrl('git@github.com:gatewaze/example-publish.git', 'github')).toBe(
      'git@github.com:gatewaze/example-publish.git',
    );
  });
});
  ...
  it('rejects mixed single + separate URLs', async () => {
    ...
    await expect(
      graduateNewsletterToExternal(
        {
          collectionId: 'c1',
          collection: { name: 'EXAMPLE', slug: 'example' },
