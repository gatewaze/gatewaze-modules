...
describe('toSshPushUrl', () => {
  it('rewrites github https → ssh', () => {
    expect(toSshPushUrl('https://github.com/gatewaze/aaif-publish.git')).toBe(
      'git@github.com:gatewaze/aaif-publish.git',
    );
  });

  it('rewrites github https without .git suffix', () => {
    expect(toSshPushUrl('https://github.com/gatewaze/aaif-publish')).toBe(
      'git@github.com:gatewaze/aaif-publish.git',
    );
  });
  ...
  it('leaves already-ssh urls unchanged', () => {
    expect(toSshPushUrl('git@github.com:gatewaze/aaif-publish.git')).toBe(
      'git@github.com:gatewaze/aaif-publish.git',
    );
  });
