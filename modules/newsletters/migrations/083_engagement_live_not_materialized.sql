-- ============================================================================
-- Module: newsletters
-- Migration: 083_engagement_live_not_materialized
-- Description: Fix the newsletter engagement LIVE query planner blowup. The
-- `sends` CTE was materialized (referenced by several later CTEs), which hid the
-- specific newsletter_send_ids from the planner. It then estimated the
-- per-send recipient count as the table AVERAGE (~28k) instead of the edition's
-- actual few-thousand, and chose a full Seq Scan of all ~1M email_interactions
-- for the human/machine classification join — making even a small edition take
-- 25s+ (statement-timeout territory). This is the function the snapshot cache is
-- built on, so any uncached edition (recent sends) or a cache refresh was
-- crawling.
--
-- Marking `sends` NOT MATERIALIZED lets the edition filter propagate to the
-- big-table joins (index nested-loops), cutting a single-edition compute from
-- 25s+ to ~1.5s. Same results; planner-only change.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.newsletter_edition_engagement_live(p_edition_ids uuid[])
 RETURNS TABLE(edition_id uuid, sent bigint, delivered bigint, unique_opens bigint, unique_clicks bigint, human_opens bigint, human_clicks bigint, machine_opens bigint, machine_clicks bigint, human_source text, bounced bigint, unsubscribed bigint, suppressed bigint, cio_human_opens bigint, cio_machine_opens bigint, cio_human_clicks bigint)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET statement_timeout TO '300000'
AS $function$
  WITH consts AS (SELECT 0.7265::numeric AS g, 0.0212::numeric AS r),
  sends AS NOT MATERIALIZED (
