-- ============================================================================
-- newsletters module — fix newsletters_ck_enqueue trigger to bypass RLS
--
-- The original definition in 017_keyword_adapter.sql was plain SECURITY INVOKER,
-- so when a non-service_role user (e.g. an admin saving a newsletter edition
-- via the editor) caused an INSERT/UPDATE on `newsletters_editions`, the AFTER
-- trigger fired and tried to INSERT into `content_keyword_match_queue` — which
-- has an RLS policy that only permits service_role writes. Result: 42501
-- ("new row violates row-level security policy for table
-- content_keyword_match_queue").
--
...
--
-- Mirrors events/009_fix_keyword_trigger_definer.sql.
-- ============================================================================
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE '[newsletters/026_fix_keyword_trigger_definer] content-keywords not installed; skipping';
    RETURN;
  END IF;
...
BEGIN
  ...
  -- attached to the newsletters_editions table.
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema='public' AND table_name='content_keyword_match_queue'
  ) THEN
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'gatewaze_module_writer') THEN
    ALTER FUNCTION public.newsletters_ck_enqueue() OWNER TO gatewaze_module_writer;
  END IF;
END $own$;
