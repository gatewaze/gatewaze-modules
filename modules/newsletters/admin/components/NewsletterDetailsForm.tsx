        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.from_name || ''} onChange={(e) => setForm(f => ({ ...f, from_name: e.target.value }))} className={inputClass} placeholder="EXAMPLE Newsletter" />
            </div>
            <div>
              <label className="block text-sm font-medium text-[var(--gray-11)] mb-1">From Email</label>
              <input type="email" value={form.from_email || ''} onChange={(e) => setForm(f => ({ ...f, from_email: e.target.value }))} className={inputClass} placeholder="newsletter@example.io" />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[var(--gray-11)] mb-1">Reply-To Email</label>
              <input type="email" value={form.reply_to || ''} onChange={(e) => setForm(f => ({ ...f, reply_to: e.target.value }))} className={inputClass} placeholder="hello@example.io" />
            </div>
            <div>
              <label className="block text-sm font-medium text-[var(--gray-11)] mb-1">Forward Replies To</label>
              <input type="email" value={form.forward_replies_to || ''} onChange={(e) => setForm(f => ({ ...f, forward_replies_to: e.target.value }))} className={inputClass} placeholder="team@example.io" />
