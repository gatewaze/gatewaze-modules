...
/**
 * Editor view of a templates_block_defs row.
 *
 * Field naming preserves the legacy "block_type" alias for the
 * stable per-library identifier (DB column: `key`). Queries should
 * project `key` AS `block_type` so this consumer-facing shape stays
 * stable. The legacy `content` jsonb has been split into top-level
 * columns `html`, `rich_text_template`, `schema`, `has_bricks`.
 */
export interface BlockTemplate {
  id: string;
  name: string;
  /** Legacy alias for templates_block_defs.key. */
  block_type: string;
...
export interface BrickTemplate {
  id: string;
  name: string;
  /** Legacy alias for templates_brick_defs.key. */
  brick_type: string;
  schema: Record<string, unknown>;
  html: string | null;
  rich_text_template: string | null;
...
interface BlockEditorProps {
  ...
  /** Per-newsletter overrides (e.g. helix_project_id for AI content). */
...
export function BlockEditor({
  ...
  const renderField = (fieldName: string, fieldSchema: SchemaField) => {
    ...
    if (fieldSchema.type === 'string') {
      // AI content field
      if (fieldSchema.format === 'ai_content') {
        return (
          <AiContentField
            key={fieldName}
            fieldName={fieldName}
  ...
  return (
    <div
      ...
      style={style}
      className={`
        bg-white dark:bg-gray-900 border border-primary-500
