...
import type { NewsletterEdition, EditionBlock, EditionBrick } from './types';

export type DistributionChannel = 'html' | 'substack' | 'beehiiv';
...
function getChannelSuffix(channel: DistributionChannel): string {
  switch (channel) {
    ...
    case 'beehiiv':
      return '_BH';
    default:
      return ''; // HTML is the default, no suffix
  }
}
...
export function generateEditionLinks(
  ...
  channels: DistributionChannel[] = ['html', 'substack', 'beehiiv']
): GeneratedLink[] {
