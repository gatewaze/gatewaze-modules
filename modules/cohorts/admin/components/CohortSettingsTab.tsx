...
export function CohortSettingsTab({ cohort, onUpdate }: CohortSettingsTabProps) {
  ...
  const [basicFormData, setBasicFormData] = useState({
    content_category: (cohort as any).content_category || '',
    title: cohort.title,
          ...
          <div className="space-y-4">
            ...
            {contentCategories.length > 0 && (
              <div>
                ...
                <select
                  value={basicFormData.content_category}
                  onChange={(e) => setBasicFormData({ ...basicFormData, content_category: e.target.value })}
                  className="w-full px-3 py-2 border rounded-lg dark:bg-gray-800 dark:border-gray-700"
                >
