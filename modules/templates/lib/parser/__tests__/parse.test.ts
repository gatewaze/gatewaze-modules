...
<!-- /BLOCK:ai_summary -->`;
  ...
  it('parses the demo-community.html fixture without errors', () => {
    const html = fixture('demo-community.html');
    const result = parse(html, { sourcePath: 'demo-community.html' });
    // The demo template uses some non-schema runtime globals; the parser
    ...
    expect(blockKeys).toContain('demo_community');
    expect(blockKeys).toContain('footer');
    // The Demo community block has has_bricks=true with 3 bricks
    const community = result.block_defs.find((b) => b.key === 'demo_community');
