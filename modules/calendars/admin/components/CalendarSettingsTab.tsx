...
import { CalendarCoverUploadField } from './CalendarCoverUploadField';
...
interface SettingsFormData {
  ...
  // About-page sections (HTML from RichTextEditor)
...
export function CalendarSettingsTab({ calendar, onUpdate }: CalendarSettingsTabProps) {
  ...
  const { register, handleSubmit, watch, setValue, formState: { errors, isDirty } } = useForm<SettingsFormData>({
    defaultValues: {
      name: calendar.name,
      ...
      lumaWebhookSecret: calendar.lumaWebhookSecret || '',
      ...
      aboutOrganisers: calendar.aboutOrganisers || '',
      aboutFaq: calendar.aboutFaq || '',
      aboutSponsors: calendar.aboutSponsors || '',
  ...
  const onSubmit = async (data: SettingsFormData) => {
    ...
    try {
      ...
      const updateData: UpdateCalendarInput = {
        ...
        lumaWebhookSecret: data.lumaWebhookSecret || undefined,
        externalUrl: data.externalUrl || undefined,
        settings: {
          ...calendar.settings,
          location: locationSettings,
        },
        aboutOrganisers: data.aboutOrganisers || null,
        aboutFaq: data.aboutFaq || null,
        aboutSponsors: data.aboutSponsors || null,
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <Card skin="shadow" className="p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <CalendarCoverUploadField
            calendarId={calendar.calendarId}
            value={watch('coverImageUrl')}
            onChange={(next) => setValue('coverImageUrl', next, { shouldDirty: true })}
          />
      ...
      <Card skin="shadow" className="p-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <Input
            ...
            {...register('lumaWebhookSecret')}
            placeholder="whsec-xxx"
            helperText="Webhook signing secret from Luma, used to verify incoming registration webhooks"
          />
      ...
      {/* About Page */}
      <Card skin="shadow" className="p-6">
        <h3 className="text-lg font-semibold mb-1">About Page</h3>
        <p className="text-sm text-gray-500 mb-4">
          Rich-text content shown on the public About page at <code>/calendars/{calendar.slug || calendar.calendarId}/about</code>.
          Each section renders only if it has content.
        </p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div>
            ...
            <p className="text-xs text-gray-500 mb-2">
              Who runs this calendar — bios, photos, contact links.
            </p>
            <RichTextEditor
              content={watch('aboutOrganisers')}
              onChange={(html) => setValue('aboutOrganisers', html, { shouldDirty: true })}
              placeholder="Introduce the team behind this calendar…"
            />
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mb-2">
              Frequently asked questions about events, format, code of conduct, etc.
            </p>
            <RichTextEditor
              content={watch('aboutFaq')}
              onChange={(html) => setValue('aboutFaq', html, { shouldDirty: true })}
              placeholder="What questions do attendees ask most often?"
            />
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mb-2">
              Sponsor packages, current sponsors, contact details for prospective sponsors.
            </p>
            <RichTextEditor
              content={watch('aboutSponsors')}
              onChange={(html) => setValue('aboutSponsors', html, { shouldDirty: true })}
              placeholder="Tell sponsors why they should partner with you…"
            />
          </div>
