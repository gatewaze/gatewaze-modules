...
export function CalendarEventsTab({ calendar, onRefresh }: CalendarEventsTabProps) {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor((row) => row.event?.lumaGuestCount, {
        id: 'lumaGuestCount',
        header: 'Guests',
        cell: (info) => {
          const n = info.getValue();
          return n == null ? <span className="text-gray-400">—</span> : n.toLocaleString();
        },
      }),
      // Tickets column removed — Luma's ticket_count mirrors guest_count 98% of
      // the time (190/193 events tested). Column still persisted in case the
      // data ever becomes useful; just not shown.
          ...
          return (
            <RowActions
              actions={[
                ...
                { label: event.isFeatured ? 'Unfeature' : 'Feature', icon: <StarIcon className="size-4" />, onClick: () => handleToggleFeatured(event) },
                { label: 'Remove from calendar', icon: <TrashIcon className="size-4" />, onClick: () => setRemoveEvent(event), color: 'red' },
              ]}
            />
          );
        },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Events Table — uses shared DataTable for sticky columns, horizontal
          scroll, double-click-to-open, and the right-side action dropdown
          that stays pinned when the table scrolls horizontally. */}
      <Card className="overflow-hidden">
        <DataTable
          table={table}
          loading={loading}
          onRowDoubleClick={(row) => navigate(`/events/${row.eventId}`)}
          emptyState={
