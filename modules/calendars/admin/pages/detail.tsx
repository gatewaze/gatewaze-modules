...
import { CalendarMessagingTab } from '../components/CalendarMessagingTab';
...
import { CalendarReportsTab } from '../components/CalendarReportsTab';
import { CalendarSettingsTab } from '../components/CalendarSettingsTab';
import { ChartBarIcon } from '@heroicons/react/24/outline';
import { useHasModule } from '@/hooks/useModuleFeature';
// Conversations and Speakers tabs are loaded lazily so the calendar detail
// page works even when those modules aren't installed.
import { lazy, Suspense } from 'react';
const CalendarConversationsTab = lazy(async () => {
  try {
    const mod = await import(
      /* @vite-ignore */ '../../../conversations/admin/components/CalendarConversationsTab'
    );
    return { default: mod.CalendarConversationsTab };
  } catch {
    return { default: () => null };
  }
});
const CalendarSpeakersTab = lazy(async () => {
  try {
    const mod = await import(
      /* @vite-ignore */ '../../../event-speakers/admin/components/CalendarSpeakersTab'
    );
    return { default: mod.CalendarSpeakersTab };
  } catch {
    return { default: () => null };
  }
});
...
export default function CalendarDetailPage() {
  ...
  // Define valid tabs. Feature tabs are conditionally included based on
  // which sister modules are installed.
  const messagingEnabled = useHasModule('bulk-emailing');
  const chatEnabled = useHasModule('conversations');
  const speakersEnabled = useHasModule('event-speakers');
  const validTabsArray: string[] = ['overview', 'events', 'people'];
  if (messagingEnabled) validTabsArray.push('messaging');
  if (chatEnabled) validTabsArray.push('chat');
  if (speakersEnabled) validTabsArray.push('speakers');
  validTabsArray.push('scrapers', 'reports', 'permissions', 'settings');
  type TabType = (typeof validTabsArray)[number];
  const activeTab: TabType = (tab && validTabsArray.includes(tab as TabType)) ? (tab as TabType) : 'overview';
  ...
  return (
    <Page>
      ...
      <div className="p-6 space-y-6">
        ...
        {activeTab === 'messaging' && messagingEnabled && (
          <CalendarMessagingTab calendar={calendar} />
        )}
        {activeTab === 'chat' && chatEnabled && (
          <Suspense fallback={<div className="flex justify-center py-12"><LoadingSpinner /></div>}>
            <CalendarConversationsTab calendar={calendar} />
          </Suspense>
        )}
        {activeTab === 'speakers' && speakersEnabled && (
          <Suspense fallback={<div className="flex justify-center py-12"><LoadingSpinner /></div>}>
            <CalendarSpeakersTab calendar={calendar} />
          </Suspense>
        )}
        {activeTab === 'reports' && (
          <CalendarReportsTab calendar={calendar} stats={stats} />
        )}
