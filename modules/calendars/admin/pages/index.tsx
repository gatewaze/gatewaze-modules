...
export default function CalendarsPage() {
  ...
  // Per-admin scoping: super-admins see everything, calendar-scoped admins
  // see only the calendars in their `admin_calendar_permissions`. The hook
  // returns `calendarIds = null` for super-admins (no filter applied).
  const { isSuperAdmin, calendarIds, loading: scopeLoading } = useAdminScope();
  ...
  const loadCalendars = async () => {
    try {
      ...
      if (result.success && result.data) {
        let visible = result.data.calendars;
        // Apply scope filter for non-super-admins
        if (!isSuperAdmin && calendarIds !== null) {
  ...
  useEffect(() => {
    // Wait until scope is loaded so the first render filters correctly
    if (!scopeLoading) {
      loadCalendars();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [scopeLoading, isSuperAdmin, calendarIds]);
      ...
      <div className="space-y-6">
        ...
        <Card className="overflow-hidden">
          <DataTable
            table={table}
            loading={loading}
            onRowDoubleClick={(calendar) => navigate(`/calendars/${calendar.id}`)}
            emptyState={
              <div>
      ...
      >
        <div className="space-y-4">
          ...
          <Input
            ...
            placeholder="whsec-xxx (optional)"
