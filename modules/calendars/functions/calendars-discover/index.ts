...
export default async function(req: Request) {
  ...
  try {
    ...
    const brand = body.brand || 'auto'; // Default to auto-discovery

    // 'manual' brands should not use auto-discovery
    if (brand === 'manual' && (mode === 'discover' || mode === 'create_scraper')) {
      return new Response(
        JSON.stringify({
          error: 'Calendar discovery is only available for auto-discovery brands. Manually-managed calendars are configured by hand.',
          hint: 'Manually-managed brands use curated calendars that are configured by hand.'
        }),
