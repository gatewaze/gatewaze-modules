...
describe('stream-keys', () => {
  ...
  it('derives brand-prefixed keys', async () => {
    ...
    expect(mod.recipeRunStreamKey('uuid-1')).toMatch(/^aaif:ai:run:uuid-1$/);
    expect(mod.threadStreamKey('uuid-2')).toMatch(/^aaif:ai:thread:uuid-2$/);
    expect(mod.recipeRunCancelChannel('uuid-3')).toMatch(/^aaif:ai:cancel:run:uuid-3$/);
    expect(mod.messageCancelChannel('uuid-4')).toMatch(/^aaif:ai:cancel:msg:uuid-4$/);
    expect(mod.useCaseSemaphoreKey('daily-briefing')).toMatch(/^aaif:ai:semaphore:use_case:daily-briefing$/);
  });
