...
const GENERATE_STEPS = [
  ...
  { label: 'Analysing profiles…', minMs: 2000 },
  { label: 'AI is pairing attendees…', minMs: 5000 },
  { label: 'Validating matches…', minMs: 15000 },
  { label: 'Saving results…', minMs: 25000 },
];
...
// ── Props (injected by module slot system) ───────────────────────────────────
...
export default function EventMatchingTab({ eventId }: Props) {
  ...
  const loadMatches = useCallback(async () => {
    setLoading(true);
    try {
      const [evRes, matchRes, settingsRes] = await Promise.all([
        ...
        supabase
          ...
          .select('id, from_address, from_key, reply_to, template_id, subject, content')
          .eq('event_id', eventId)
          .single(),
      ]);
      ...
      if (settingsRes.data) {
        ...
        setFromAddress(settingsRes.data.from_address ?? '');
        setReplyTo(settingsRes.data.reply_to ?? '');
        setTemplateId(settingsRes.data.template_id ?? null);

        if (settingsRes.data.subject || settingsRes.data.content) {
          setEmailSubject(settingsRes.data.subject ?? '');
          setEmailContent(settingsRes.data.content ?? '');
        } else if (settingsRes.data.template_id) {
          const { data: tmpl } = await supabase
            .from('email_templates')
            .select('subject, html_body')
            .eq('id', settingsRes.data.template_id)
            .single();
          if (tmpl) {
            setEmailSubject(tmpl.subject);
            setEmailContent(tmpl.html_body);
          }
        }
      }
  ...
  const handleTemplateSelect = async (selectedTemplateId: string) => {
    ...
    if (selectedTemplateId) {
      ...
      if (template) {
        ...
        setEmailContent(template.html_body);
      }
    } else {
      setEmailSubject('');
      setEmailContent('');
  ...
  const doSendEmails = async () => {
    ...
    try {
      ...
      if (allErrors.length > 0) {
        toast.error(`${allErrors.length} error${allErrors.length !== 1 ? 's' : ''} during send`);
      }
      await loadMatches();
    } catch (err: any) {
            ...
            <div className="space-y-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              ...
              <div>
                ...
                <input
                  type="email"
                  value={fromAddress}
                  onChange={(e) => { setFromAddress(e.target.value); setSettingsDirty(true); }}
        ...
        {!sending ? (
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              ...
              <strong className="text-gray-900 dark:text-white">{progress.total}</strong> emails&hellip;
            </p>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">Starting send&hellip;</p>
        )}
