import { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router';
import {
  ArrowPathIcon,
  UserGroupIcon,
  TrophyIcon,
  MagnifyingGlassIcon,
  ArrowLeftIcon,
  ArrowDownTrayIcon,
} from '@heroicons/react/24/outline';
import { Card, Button } from '@/components/ui';
import { Page } from '@/components/shared/Page';
import { PersonProfile } from '@/utils/winnerSelectionService';
import WinnerSelectionModal from '@/components/WinnerSelectionModal';
import { CompetitionDiscountService, EventCompetitionWithEvent } from '@/utils/competitionDiscountService';
import { useAccountAccess } from '@/hooks/useAccountAccess';

export default function CompetitionEntriesPage() {
  const { competitionId } = useParams<{ competitionId: string }>();
  const navigate = useNavigate();
  const { isAccountUser, isSystemAdmin, highestAccountRole, canEdit } = useAccountAccess();

  const [competition, setCompetition] = useState<EventCompetitionWithEvent | null>(null);
  const [entriesCustomers, setEntriesCustomers] = useState<PersonProfile[]>([]);
  const [isLoadingEntries, setIsLoadingEntries] = useState(false);
  const [entriesSearchTerm, setEntriesSearchTerm] = useState<string>('');
  const [selectedEntries, setSelectedEntries] = useState<Set<string>>(new Set());
  const [currentCyclingEmail, setCurrentCyclingEmail] = useState<string>('');
  const [isWinnerModalOpen, setIsWinnerModalOpen] = useState(false);

  useEffect(() => {
    console.log('Competition Entries Page - competitionId:', competitionId);
    if (competitionId) {
      loadCompetition(competitionId);
      loadEntries(competitionId);
    }
  }, [competitionId]);

  const loadCompetition = async (id: string) => {
    try {
      console.log('Loading competition with UUID:', id);
      const response = await CompetitionDiscountService.getCompetition(id);
      console.log('Competition loaded:', response);
      if (response.success && response.data) {
        setCompetition(response.data);
      } else {
        console.error('Competition not found');
        setCompetition({} as EventCompetitionWithEvent); // Set empty object to prevent infinite loading
      }
    } catch (error) {
      console.error('Error loading competition:', error);
      setCompetition({} as EventCompetitionWithEvent); // Set empty object to prevent infinite loading
    }
  };

  const loadEntries = async (id: string) => {
    setIsLoadingEntries(true);
    setEntriesCustomers([]);
    setSelectedEntries(new Set());

    try {
      console.log('📊 Fetching competition entries from competition_entries table...');
      const response = await CompetitionDiscountService.getCompetitionEntriesWithCustomerDetails(id);

      if (!response.success) {
        console.error('❌ Error loading entries:', response.error);
        setIsLoadingEntries(false);
        return;
      }

      const customers = response.data || [];
      console.log(`✅ Fetched ${customers.length} entries from database`);
      console.log(`🎉 Successfully loaded ${customers.length} entries`);
      setEntriesCustomers(customers);
    } catch (error) {
      console.error('❌ Fatal error loading entries:', error);
      if (error instanceof Error) {
        console.error('   Error details:', {
          name: error.name,
          message: error.message,
          stack: error.stack
        });
      }
    } finally {
      setIsLoadingEntries(false);
    }
  };

  const handleToggleEntry = (email: string) => {
    setSelectedEntries(prev => {
      const newSet = new Set(prev);
      if (newSet.has(email)) {
        newSet.delete(email);
      } else {
        newSet.add(email);
      }
      return newSet;
    });
  };

  const handleSelectAll = () => {
    if (selectedEntries.size === entriesCustomers.length) {
      setSelectedEntries(new Set());
    } else {
      setSelectedEntries(new Set(entriesCustomers.map(c => c.email)));
    }
  };

  const handleSelectWinners = () => {
    const selectedCount = selectedEntries.size;

    if (selectedCount === 0) {
      // No selection - open modal for random selection
      handleWinnerModalOpen();
    } else if (selectedCount === 1) {
      // Single selection - open modal with that winner selected
      const email = Array.from(selectedEntries)[0];
      handleWinnerModalOpen(email);
    } else {
      // Multiple selections - open modal in "Winner Selected" state without showing specific user
      handleWinnerModalOpen('MULTIPLE_WINNERS');
    }
  };

  const handleExportCSV = () => {
    // Define CSV columns - matching members page export
    const columns = [
      'email',
      'first_name',
      'last_name',
      'job_title',
      'company',
      'linkedin_url',
      'city',
      'country',
      'continent'
    ];

    // Create CSV header
    const csvHeader = columns.join(',');

    // Get customers to export
    const customersToExport = selectedEntries.size > 0
      ? entriesCustomers.filter(c => selectedEntries.has(c.email))
      : entriesCustomers;

    // Create CSV rows
    const csvRows = customersToExport.map(customer => {
      const row = [
        customer.email || '',
        customer.first_name || '',
        customer.last_name || '',
        customer.job_title || '',
        customer.company || '',
        customer.linkedin_url || '',
        customer.city || '',
        customer.country || '',
        customer.continent || ''
      ];

      // Escape fields that contain commas, quotes, or newlines
      return row.map(field => {
        const fieldStr = String(field);
        if (fieldStr.includes(',') || fieldStr.includes('"') || fieldStr.includes('\n')) {
          return `"${fieldStr.replace(/"/g, '""')}"`;
        }
        return fieldStr;
      }).join(',');
    });

    // Combine header and rows
    const csvContent = [csvHeader, ...csvRows].join('\n');

    // Create blob and download
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `competition_entries_${competition?.eventTitle?.replace(/[^a-z0-9]/gi, '_')}_${new Date().toISOString().split('T')[0]}.csv`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  const handleWinnerModalOpen = (email?: string) => {
    if (email) {
      setCurrentCyclingEmail(email);
    }
    setIsWinnerModalOpen(true);
  };

  const handleWinnerModalClose = () => {
    setIsWinnerModalOpen(false);
    setCurrentCyclingEmail('');
  };

  const formatDate = (dateString: string) => {
    const date = new Date(dateString);
    return date.toLocaleDateString('en-US', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
    });
  };

  const filteredCustomers = entriesCustomers.filter(customer =>
    customer.email.toLowerCase().includes(entriesSearchTerm.toLowerCase())
  );

  if (!competition || !competition.eventTitle) {
    return (
      <Page title="Competition Entries">
        <div className="p-6">
          <div className="text-center py-12">
            <ArrowPathIcon className="size-12 mx-auto text-neutral-400 animate-spin mb-3" />
            <p className="text-neutral-600 dark:text-neutral-400">Loading competition...</p>
          </div>
        </div>
      </Page>
    );
  }

  return (
    <Page title={`Entries - ${competition.eventTitle || 'Competition'}`}>
      <div className="p-6 space-y-6">
        {/* Back Button */}
        <div>
          <Button
            onClick={() => navigate('/competitions')}
            variant="outline"

          >
            <ArrowLeftIcon className="size-4" />
            Back to Competitions
          </Button>
        </div>

        {/* Header */}
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-semibold text-[var(--gray-12)]">
              Competition Entries
            </h1>
            {isAccountUser && !canEdit && (
              <span className="px-3 py-1 text-xs font-medium bg-neutral-100 dark:bg-neutral-700 text-neutral-600 dark:text-neutral-400 rounded-full">
                View Only
              </span>
            )}
          </div>
          <p className="text-sm text-neutral-600 dark:text-neutral-400 mt-1">
            {competition.eventTitle}
          </p>
          <p className="text-xs text-neutral-500 dark:text-neutral-500 mt-1">
            {competition.eventCity && competition.eventCountryCode && (
              <>{competition.eventCity}, {competition.eventCountryCode} • </>
            )}
            {competition.eventStart && formatDate(competition.eventStart)}
          </p>
        </div>

        {/* Stats Card */}
        <Card variant="surface" className="p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="text-sm text-neutral-600 dark:text-neutral-400">
                {entriesCustomers.length > 0 ? `${filteredCustomers.length} of ${entriesCustomers.length} entries` : 'Loading entries...'}
                {selectedEntries.size > 0 && (
                  <span className="ml-2 text-primary-600 dark:text-primary-400 font-medium">
                    ({selectedEntries.size} selected)
                  </span>
                )}
              </div>
              {entriesCustomers.length > 0 && (
                <Button
                  variant="ghost"
                  onClick={handleSelectAll}
                >
                  {selectedEntries.size === entriesCustomers.length ? 'Deselect All' : 'Select All'}
                </Button>
              )}
            </div>
            <div className="flex items-center gap-2">
              {isAccountUser && !isSystemAdmin ? (
                // Account users see export button only
                <Button
                  onClick={handleExportCSV}
                  disabled={entriesCustomers.length === 0}
      
                  variant="outline"
                >
                  <ArrowDownTrayIcon className="size-5" />
                  Export {selectedEntries.size > 0 ? `${selectedEntries.size} Selected` : 'All'} to CSV
                </Button>
              ) : (
                // System admins see winner selection
                <>
                  <Button
                    onClick={handleExportCSV}
                    disabled={entriesCustomers.length === 0}
        
                    variant="outline"
                  >
                    <ArrowDownTrayIcon className="size-5" />
                    Export to CSV
                  </Button>
                  <Button
                    onClick={handleSelectWinners}
                    disabled={entriesCustomers.length === 0}
        
                  >
                    <TrophyIcon className="size-5" />
                    {selectedEntries.size > 0
                      ? `Select ${selectedEntries.size} Winner${selectedEntries.size !== 1 ? 's' : ''}`
                      : 'Select Winners'
                    }
                  </Button>
                </>
              )}
            </div>
          </div>
        </Card>

        {/* Search */}
        <Card variant="surface" className="p-4">
          <div className="relative">
            <MagnifyingGlassIcon className="absolute left-3 top-1/2 -translate-y-1/2 size-5 text-neutral-400" />
            <input
              type="text"
              placeholder="Filter by email address..."
              value={entriesSearchTerm}
              onChange={(e) => setEntriesSearchTerm(e.target.value)}
              className="w-full pl-10 pr-4 py-2 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500 text-neutral-900 dark:text-white"
            />
          </div>
        </Card>

        {/* Entries List */}
        <Card variant="surface" className="p-6">
          {isLoadingEntries ? (
            <div className="text-center py-12">
              <ArrowPathIcon className="size-12 mx-auto text-neutral-400 animate-spin mb-3" />
              <p className="text-neutral-600 dark:text-neutral-400">Loading entries...</p>
            </div>
          ) : entriesCustomers.length === 0 ? (
            <div className="text-center py-12">
              <UserGroupIcon className="size-12 mx-auto text-neutral-400 mb-3" />
              <p className="text-neutral-600 dark:text-neutral-400">No entries found for this competition</p>
            </div>
          ) : filteredCustomers.length === 0 ? (
            <div className="text-center py-12">
              <UserGroupIcon className="size-12 mx-auto text-neutral-400 mb-3" />
              <p className="text-neutral-600 dark:text-neutral-400">No entries match your search</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredCustomers.map((customer, idx) => {
                const isSelected = selectedEntries.has(customer.email);
                const fullName = [customer.first_name, customer.last_name].filter(Boolean).join(' ') || 'No name';

                return (
                  <div
                    key={`${customer.email}-${idx}`}
                    onClick={() => handleToggleEntry(customer.email)}
                    className={`relative p-4 rounded-lg border-2 cursor-pointer transition-all ${
                      currentCyclingEmail === customer.email
                        ? 'bg-yellow-100 dark:bg-yellow-900/20 border-yellow-500'
                        : isSelected
                        ? 'bg-primary-50 dark:bg-primary-900/20 border-primary-500'
                        : 'bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 hover:border-primary-300 dark:hover:border-primary-600'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="font-semibold text-neutral-900 dark:text-white text-base">
                        {fullName}
                      </h3>
                      <input
                        type="checkbox"
                        checked={isSelected}
                        onChange={(e) => e.stopPropagation()}
                        className="size-5 rounded border-gray-300 text-primary-600 focus:ring-primary-500 cursor-pointer flex-shrink-0"
                      />
                    </div>
                    <div className="space-y-1">
                      <p className="text-sm text-neutral-600 dark:text-neutral-400 font-mono truncate">
                        {customer.email}
                      </p>
                      {customer.job_title && customer.company && (
                        <p className="text-sm text-neutral-500 dark:text-neutral-500">
                          {customer.job_title} at {customer.company}
                        </p>
                      )}
                      {customer.job_title && !customer.company && (
                        <p className="text-sm text-neutral-500 dark:text-neutral-500">
                          {customer.job_title}
                        </p>
                      )}
                      {!customer.job_title && customer.company && (
                        <p className="text-sm text-neutral-500 dark:text-neutral-500">
                          {customer.company}
                        </p>
                      )}
                      {customer.city && (
                        <p className="text-sm text-neutral-500 dark:text-neutral-500">
                          📍 {customer.city}
                        </p>
                      )}
                    </div>
                  </div>
                );
              })}
            </div>
          )}
        </Card>
      </div>

      {/* Winner Selection Modal - Only for system admins */}
      {!isAccountUser && competition && competition.eventTitle && (
        <WinnerSelectionModal
          isOpen={isWinnerModalOpen}
          onClose={handleWinnerModalClose}
          customers={entriesCustomers}
          competitionTitle={competition.eventTitle || 'Competition'}
          onCyclingEmailChange={setCurrentCyclingEmail}
          eventStart={competition.eventStart || ''}
          eventEnd={competition.eventEnd || ''}
          offerTicketDetails={competition.offerTicketDetails || ''}
          preSelectedEmail={currentCyclingEmail}
          eventId={competition.eventId || eventId || ''}
          selectedEmails={Array.from(selectedEntries)}
        />
      )}
    </Page>
  );
}
