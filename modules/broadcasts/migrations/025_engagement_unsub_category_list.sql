-- ============================================================================
-- Module: broadcasts
-- Migration: 025_engagement_unsub_category_list
-- Description: Unsubscribed/Suppressed always showed 0 for segment/category
-- broadcasts. The unsubs/supp CTEs only looked at the send's list_ids array,
-- but a segment or category send has list_ids = {} and tracks its list in
-- category_list_id — so there was no list to match opt-outs against. Include
-- category_list_id alongside list_ids in both CTEs. (Opt-outs are still
-- intersected with this send's actual recipients and its start time, so a
-- shared category list doesn't over-count someone who never got this send.)
-- ============================================================================
...
AS $function$
  ...
  unsubs AS (
    ...
    WHERE ls.subscribed = false
      ...
      AND ls.list_id IN (
        SELECT unnest(list_ids)::uuid FROM sends WHERE COALESCE(array_length(list_ids, 1), 0) > 0
        UNION
        SELECT category_list_id FROM sends WHERE category_list_id IS NOT NULL
      )
