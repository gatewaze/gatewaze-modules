...
export class LumaUploadService {
   ...
   *
   * Routes through the API server (`/api/modules/invoke-function/...`) so the
   * request uses the server-side SUPABASE_URL. This works in both self-hosted
   * mode (local edge runtime) and cloud mode (Supabase Cloud, after
   * `make deploy-functions`).
   */
  static async triggerBackgroundProcessing(uploadId: string): Promise<void> {
    const apiUrl = import.meta.env.VITE_API_URL ?? '';
    const functionName = 'integrations-luma-process-csv';
    ...
    try {
      ...
      const res = await fetch(
        `${apiUrl}/api/modules/invoke-function/${functionName}`,
        {
          method: 'POST',
          headers,
    ...
    if (error) {
      ...
      await supabase
        ...
        .update({
          status: 'failed',
          errors: [{ row: 0, error: `Failed to start processing: ${error}` }],
        })
        .eq('id', uploadId);
