...
/**
 * Local requireJwt for the send-testing module.
 *
 ...
 * gate for send-testing's admin API, which can create and delete tens of
 * thousands of people rows and read recipient addresses. It must verify token
 * signatures — trusting an unverified
...
function getJwtSecret(): string {
  ...
  if (!secret) {
    throw new Error('JWT secret (SUPABASE_JWT_SECRET/JWT_SECRET) not set; send-testing requireJwt cannot verify tokens');
  }
  return secret;
}
